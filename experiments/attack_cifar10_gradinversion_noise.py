...
def run_attack(pipeline, attack_hparams):
    ...
    for batch_idx, (batch_inputs, batch_targets) in enumerate(trainloader):
        ...
        for grad in batch_gradients:
            #print(grad)
            #grad = grad / torch.maximum(torch.ones(grad.size()).to(DEVICE), torch.norm(grad) )
            grad += torch.randn_like(grad).to(DEVICE) * (attack_hparams["noise_multiplier"])
            #grad.add_(torch.randn(grad.size()).to(DEVICE) * (attack_hparams["noise_multiplier"]))
            #print(grad)
